...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) !=0:
        ...
        preds = model.predict(batch_size_parses)
        for parse1, pred in zip(batch_size_parses, preds):
            parse1.parse([pred])
            if len(parse1.buffer) == 0 and len(parse1.stack) == 1:
                unfinished_parses.remove(parse1)
